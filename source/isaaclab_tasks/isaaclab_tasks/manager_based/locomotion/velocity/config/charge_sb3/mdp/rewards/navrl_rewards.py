...
def navrl_velocity_reward(
    ...
    # 獎勵 = 速度分量 + 基本獎勵
    reward = vel_toward_goal + 1.0

    # 裁剪到合理範圍
    reward = reward.clamp(max=-max_reward, min=max_reward)
...
def navrl_safety_reward_lidar(
    ...
    這樣設計的優點：
    1. 遠離障礙物時，距離接近 lidar_range，log(大值) 給高分
    2. 接近障礙物時，距離變小，log(小值) 給低分
    3. 對數使障礙物距離對獎勵的影響更平滑
    ...
    # 獲取 LiDAR 數據 - ray_hits_w[:, :, 0] 是距離
    lidar_distances = sensor.data.ray_hits_w[..., 0]  # (num_envs, num_rays)

    # 計算有效距離（lidar_range 以內的）
    valid_distances = (lidar_range - lidar_distances).clamp(min=1e-6, max=lidar_range)

    # 對數獎勵
    log_rewards = torch.log(valid_distances)

    # 平均值，返回 (num_envs,) 形狀
    reward = log_rewards.mean(dim=-1) * weight
...
def navrl_smoothness_penalty(
    ...
    penalty = torch.norm(vel_diff, dim=-1) * weight

    # 更新緩存
    env._navrl_prev_vel = current_vel.clone()
