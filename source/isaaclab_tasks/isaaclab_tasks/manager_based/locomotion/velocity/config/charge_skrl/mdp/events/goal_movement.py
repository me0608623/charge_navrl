...
def move_goal_positions(
    ...
    if goal_move_behavior == "random_walk":
        ...
        obs_xy, obs_vis = _gather_obstacle_positions(env)
        if obs_xy is not None and obs_xy.shape[1] > 0:
            # obs_xy: [N_envs, K, 2], obs_vis: [N_envs, K]
            goal_2d = goal_pos[:, :2]  # [N_envs, 2]（每 env 的 current goal）

            # 計算每個 env 的 goal 到每個 obstacle 的距離
            diff = obs_xy - goal_2d.unsqueeze(1)  # [N, K, 2]
            dists = diff.norm(dim=2)  # [N, K]

            # 不可見的 obstacle 設成很遠
            dists[~obs_vis] = 999.0

            # 每個 env 找最近 visible obstacle
            nearest_idx = dists.argmin(dim=1)  # [N]
            batch_idx = torch.arange(goal_2d.shape[0], device=device)
            nearest_obs = obs_xy[batch_idx, nearest_idx]  # [N, 2]
