"""障礙物狀態管理模組 (Obstacle State Manager)

管理障礙物的全局元數據與動態移動狀態，供 observations、events、rewards 跨模組共享。

功能分為兩大類：
1. 元數據 (Metadata)：障礙物數量、尺寸 — 在 env cfg 初始化時由 set_obstacle_metadata() 設定
2. 動態狀態 (Dynamic State)：移動障礙物的起始位置、方向向量、目標位置 — 在 reset/move 事件中更新

訓練流程整合：
  startup → set_obstacle_metadata(num, sizes)  # charge_env_cfg_vlp16.py 呼叫
  reset   → reset_obstacle_targets()            # 重置動態障礙物移動狀態
  step    → get_obstacle_metadata()             # observations 讀取尺寸/數量
          → get_obstacle_directions()           # obstacles.py 讀取移動方向

設計原則：
  - 使用模組級全域變數而非 env 屬性，避免修改 scene cfg
  - 所有 getter 函數在未初始化時返回安全預設值（不會拋出異常）
  - 動態狀態使用 dict[int, Tensor] 以 env_id 為 key，支援 per-env 重置
"""
