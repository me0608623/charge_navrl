...
SKRL 整合:
    在 SKRL 的 model 中使用此模組作為 shared feature extractor：
        extractor = NavFeatureExtractor(
            observation_space=env.observation_space,
            lidar_dim=72,
            kinematics_dim=10,
            obstacle_dim=30,
        )
    輸出 80 維特徵向量，供 Actor/Critic 網路使用。
"""
...
class NavFeatureExtractor(nn.Module):
    ...
    純 PyTorch nn.Module，不依賴任何 RL 框架（SB3/SKRL）。
    ...
    def __init__(
        ...
        # 融合後的總輸出維度
        self.features_dim = lidar_embed_dim + obstacle_embed_dim + kinematics_embed_dim
