"""基礎觀測函數 — Isaac Lab ObservationManager 格式

VLP16 訓練使用的函數:
    goal_position_in_robot_frame: 目標在機器人 body frame 的 (x,y) — 2D
    goal_distance: 到目標的歐式距離 — 1D
    base_velocity_xy: 線速度 (vx, vy) — 2D
    base_angular_velocity_z: yaw 角速度 ω_z — 1D
    dynamic_obstacles_state: Critic 特權觀測，10 個障礙物完整狀態 — 50D

函數簽名 (Isaac Lab 規範):
    def func(env: ManagerBasedRLEnv, **params) -> torch.Tensor
    返回: [num_envs, obs_dim]

座標系:
    - 世界座標 (w): Isaac Sim 全域座標
    - 機器人座標 (body frame): 以機器人為原點，前方為 x 軸
    - goal_position_in_robot_frame 使用 body frame（真實可部署）
"""
...
def discrete_last_action(env: ManagerBasedRLEnv) -> torch.Tensor:
    """離散動作空間專用的上一步動作觀測

    讀取 ActionTerm 的 processed_actions（已轉換為實際物理量），
    歸一化到 [-1, 1] 範圍。

    與 safe_last_action 的差異：
    - safe_last_action 讀取 action_manager.action（raw NN output）
      離散時是索引 0-20，clamp(-1,1) 後 2-20 全變 1.0，agent 無法區分
    - discrete_last_action 讀取 processed_actions（物理速度），正確歸一化

    Returns:
        [num_envs, 2]:
          [:, 0] = linear_velocity / max_v → [0, 1]
          [:, 1] = angular_velocity / max_ω → [-1, 1]
    """
    import math

    term = list(env.action_manager._terms.values())[0]
    pa = term.processed_actions  # [num_envs, 2]: [velocity, omega]

    max_v = 0.4               # m/s (DiscreteDifferentialDriveActionCfg.max_linear_velocity)
    max_w = math.pi / 15.0    # rad (DiscreteDifferentialDriveActionCfg.omega_range max)

    result = torch.zeros_like(pa)
    result[:, 0] = pa[:, 0] / max_v       # [0, 0.4] → [0, 1]
    result[:, 1] = pa[:, 1] / max_w       # [-π/15, π/15] → [-1, 1]

    result = torch.nan_to_num(result, nan=0.0, posinf=1.0, neginf=-1.0)
    result = torch.clamp(result, -1.0, 1.0)
    check_finite("discrete_last_action", result, raise_on_error=True)
...
# ============================================================================
# v2 觀測：ego state (4D) — acceleration, velocity, angular vel, radius
# ============================================================================

def normalized_linear_velocity(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    max_linear_velocity: float = 1.0,
) -> torch.Tensor:
    """歸一化前進速度（純量）— ego state 的 v̄_t

    回傳機器人 body-frame 的前進速度 v_x，歸一化至 [-1, 1]。
    差速驅動機器人主要沿 x 軸前進，所以 v_x 即為行進速度。

    Args:
        env: 環境實例
        asset_cfg: 機器人資產配置
        max_linear_velocity: 歸一化上限 (m/s)

    Returns:
        [num_envs, 1]: 歸一化前進速度
    """
    asset: Articulation = env.scene[asset_cfg.name]
    vel_w = torch.nan_to_num(asset.data.root_lin_vel_w[:, :3], nan=0.0, posinf=10.0, neginf=-10.0)
    quat_w = torch.nan_to_num(asset.data.root_quat_w, nan=0.0, posinf=1.0, neginf=-1.0)
    quat_w = quat_w / (torch.norm(quat_w, dim=1, keepdim=True) + 1e-6)
    vel_b = math_utils.quat_apply_inverse(quat_w, vel_w)
    vx = vel_b[:, 0:1] / max_linear_velocity
    vx = torch.nan_to_num(vx, nan=0.0, posinf=1.0, neginf=-1.0)
    vx = torch.clamp(vx, -1.0, 1.0)
    check_finite("normalized_linear_velocity", vx, raise_on_error=True)
    return vx


def normalized_linear_acceleration(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    max_acceleration: float = 1.0,
) -> torch.Tensor:
    """歸一化前進加速度（純量）— ego state 的 ā_t

    利用上一步的速度快取計算加速度：a = (v_t - v_{t-1}) / dt。
    第一步（無歷史）回傳 0。

    Args:
        env: 環境實例
        asset_cfg: 機器人資產配置
        max_acceleration: 歸一化上限 (m/s²)
    ...
    quat_w = quat_w / (torch.norm(quat_w, dim=1, keepdim=True) + 1e-6)
    vel_b = math_utils.quat_apply_inverse(quat_w, vel_w)
    vx_curr = vel_b[:, 0]  # [num_envs]

    # 初始化快取
    if not hasattr(env, "_prev_vx_for_accel"):
        env._prev_vx_for_accel = torch.zeros(env.num_envs, device=env.device)

    # 計算加速度
    dt = env.step_dt if hasattr(env, "step_dt") else (env.cfg.sim.dt * env.cfg.decimation)
    accel = (vx_curr - env._prev_vx_for_accel) / (dt + 1e-6)

    # 重置的環境：加速度歸零
    if hasattr(env, "episode_length_buf"):
        reset_mask = env.episode_length_buf == 0
        accel[reset_mask] = 0.0

    # 更新快取
    env._prev_vx_for_accel = vx_curr.clone()

    # 歸一化
    result = (accel / max_acceleration).unsqueeze(1)  # [num_envs, 1]
    result = torch.nan_to_num(result, nan=0.0, posinf=1.0, neginf=-1.0)
    result = torch.clamp(result, -1.0, 1.0)
    check_finite("normalized_linear_acceleration", result, raise_on_error=True)
    return result


def robot_radius_obs(
    env: ManagerBasedRLEnv,
    radius: float = 0.3,
) -> torch.Tensor:
    """機器人半徑常數觀測 — ego state 的 r_a

    提供固定的機器人碰撞半徑，讓 policy 感知自身大小。
    雖然是常數，但作為觀測輸入可以支援未來多尺寸機器人遷移學習。
    ...
    """
    return torch.full(
        (env.num_envs, 1), radius, device=env.device, dtype=torch.float32
    )
...
def dynamic_obstacles_state(
    ...
    # 取得障礙物尺寸
    from ..events.state import get_obstacle_sizes
