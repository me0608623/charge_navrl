...
def goal_distance(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    check_finite("goal_distance", distance, raise_on_error=True)
...
def base_angular_velocity_z(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """角速度觀測（yaw rate / ω_z）
    ...
    Returns:
        shape [num_envs, 1]: 角速度 ω_z (rad/s)
        正值 = 逆時針旋轉（左轉）
        負值 = 順時針旋轉（右轉）
    """
    asset: Articulation = env.scene[asset_cfg.name]
    ...
    # 安全處理
    omega_z = torch.nan_to_num(omega_z, nan=0.0, posinf=5.0, neginf=-5.0)
    omega_z = torch.clamp(omega_z, -5.0, 5.0)  # 限制在合理範圍

    check_finite("base_angular_velocity_z", omega_z, raise_on_error=True)
