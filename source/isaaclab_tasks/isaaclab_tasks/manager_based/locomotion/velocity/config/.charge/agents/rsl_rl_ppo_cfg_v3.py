"""Phase 3 PPO 配置：從 Phase 2 checkpoint 繼續訓練（10 個靜態障礙物，平衡安全性和效率）
...
訓練命令（指定特定 checkpoint）：
   ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
    --task Isaac-Navigation-Charge-v2 \
    --num_envs 128 \
    --resume \
    --load_run "/home/aa/IsaacLab/logs/rsl_rl/charge_navigation_phase2/charge_phase2" \
    --checkpoint "model_15998.pt"  # 使用最新的 checkpoint

播放命令（Phase 3 訓練結果）：
    ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/play.py \
        --task Isaac-Navigation-Charge-Play-v2 \
        --num_envs 3

注意：
    - 訓練時必須加 --resume 參數！否則即使配置文件設定 resume=True 也會被覆蓋
    - --load_run 支持絕對路徑，可以跨目錄載入（如從 Phase 2 載入到 Phase 3）
    - 如果不指定 --checkpoint，系統會自動使用最新的 model_*.pt 文件
    - 如果指定 --checkpoint，請確保該文件存在於 load_run 目錄中
    - Phase 3 專注於平衡安全性和效率，學習提前減速和迴避策略
    - Phase 3 使用 10 個靜態障礙物（先不使用動態障礙物）
"""
...
class ChargeNavigationPPORunnerCfgPhase3(ChargeNavigationPPORunnerCfg):
    """Phase 3 訓練配置：載入 Phase 2 checkpoint 繼續訓練（動態障礙物迴避）"""
