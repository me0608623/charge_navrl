...
from . import charge_mdp  # 自定義的 MDP 函數（觀測、獎勵、終止條件）
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    lidar = MultiMeshRayCasterCfg(
        ...
        mesh_prim_paths=[
            ...
            # 目標 3：邊界牆壁（Phase 2 新增，統一作為靜態障礙物處理）
            MultiMeshRayCasterCfg.RaycastTargetCfg(
                prim_expr="{ENV_REGEX_NS}/Wall_.*",  # 匹配所有牆壁（Wall_North, Wall_South, Wall_East, Wall_West）
...
class ActionsCfg:
    ...
    diff_drive = charge_mdp.DifferentialDriveActionCfg(
        # 差速驅動動作：控制前進速度和旋轉速度
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # ----------------------------------------------------------------
        lidar_scan = ObsTerm(
            func=charge_mdp.lidar_scan_2d_sweep,  # 觀測函數：處理 2D 掃描數據
        ...
        # ----------------------------------------------------------------
        speed = ObsTerm(
            func=charge_mdp.base_velocity_xy,  # 觀測函數：機器人座標系速度
        ...
        goal_position = ObsTerm(
            func=charge_mdp.goal_position_in_robot_frame,  # 觀測函數：計算目標相對位置
        ...
        goal_distance = ObsTerm(
            func=charge_mdp.goal_distance,  # 觀測函數：計算到目標的直線距離
            params={"asset_cfg": SceneEntityCfg("robot")},
        )
        # 輸出：[1] 個值（距離，單位：米）
...
class RewardsCfg:
    ...
    heading_to_goal = RewTerm(
        func=charge_mdp.heading_to_goal_distance_weighted,  # 獎勵函數：根據距離動態調整
    ...
    distance_to_goal = RewTerm(
        func=charge_mdp.progress_to_goal,  # 獎勵函數：進度獎勵
    ...
    reaching_goal = RewTerm(
        func=charge_mdp.reaching_goal,  # 獎勵函數：檢查是否到達
    ...
    approaching_goal = RewTerm(
        func=charge_mdp.approaching_goal_bonus,  # 獎勵函數：接近目標獎勵
    ...
    progressive_collision = RewTerm(
        func=charge_mdp.progressive_collision_penalty,  # 懲罰函數：漸進式碰撞懲罰
    ...
    # 修復後的懲罰計算：根據距離給予漸進式懲罰 [0, 1] × -0.5
    # 距離 >= SAFE_DISTANCE 米：0 分（安全，無懲罰）
    # SAFE_DISTANCE 米 > 距離 >= DANGER_DISTANCE 米：-0 到 -0.25 分（輕微懲罰）
    # DANGER_DISTANCE 米 > 距離 >= COLLISION_DISTANCE 米：-0.25 到 -0.5 分（中等懲罰）
    # 距離 < COLLISION_DISTANCE 米：-0.5 分（最大懲罰）
    # 
    # 預期效果：
    # - 累積懲罰從 -20 ~ -60 降低到 -5 ~ -15
    # - agent 可以更接近障礙物而不會受到過大懲罰
    # - 成功獎勵（+200）可以對抗累積懲罰
    ...
    hard_collision = RewTerm(
        func=charge_mdp.collision_occurred,  # 使用雷達距離判定
    ...
    tipped_over_penalty = RewTerm(
        func=charge_mdp.robot_tipped_over,  # 使用相同的翻倒判定函數
        weight=-50.0,  # 權重：-50（翻倒懲罰，比碰撞懲罰低但仍顯著）
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    # 懲罰計算：翻倒時 → -50 分（一次性）
    ...
    time_out = RewTerm(
        func=charge_mdp.time_out_penalty,  # 懲罰函數：超時檢查
...
class TerminationsCfg:
    ...
    goal_reached = DoneTerm(
        func=charge_mdp.goal_reached,  # 終止函數：檢查是否到達目標
        params={
            "asset_cfg": SceneEntityCfg("robot"),
            "threshold": GOAL_REACH_THRESHOLD,
            "body_radius": ROBOT_BODY_RADIUS,  # 使用身體半徑做補償（以「身體到目標」判定）
    ...
    collision_contact = DoneTerm(
        func=charge_mdp.collision_contact_occurred,  # 終止函數：接觸感測器判定
    ...
    collision = DoneTerm(
        func=charge_mdp.collision_occurred,  # 終止函數：雷達距離判定
        params={"sensor_cfg": SceneEntityCfg("lidar"), "threshold": COLLISION_THRESHOLD},
    ...
    tipped_over = DoneTerm(
        func=charge_mdp.robot_tipped_over,  # 終止函數：檢查翻倒
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    ...
    # ------------------------------------------------------------------------
    flying = DoneTerm(
        func=charge_mdp.robot_flying,  # 終止函數：檢查飛行
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    # 判斷：機器人高度 > 1 米 → 終止
    # 防止物理引擎出錯導致機器人「升天」
...
class EventCfg:
    ...
    reset_base = EventTerm(
        func=mdp.reset_root_state_uniform,  # 事件函數：均勻隨機重置
        mode="reset",  # 觸發模式："reset"（環境重置時）
        # 其他模式："startup"（首次啟動）、"interval"（定期）
        
        params={
            "pose_range": {
                "x": (-2.0, 2.0),  # X 座標範圍：-2 到 2 米
                "y": (-2.0, 2.0),  # Y 座標範圍：-2 到 2 米
                "yaw": (-3.14, 3.14)  # 偏航角範圍：-180° 到 +180°（任意朝向）
            },
            # 機器人會在 4m × 4m 的區域內隨機出生
    ...
    reset_obstacles = EventTerm(
        func=charge_mdp.reset_obstacles,  # 事件函數：重置所有障礙物
...
class ChargeNavigationEnvCfg_PLAY(ChargeNavigationEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.num_envs = 16  # 並行環境數：16（訓練時是 128）
        # 展示時不需要太多環境（看不過來）
