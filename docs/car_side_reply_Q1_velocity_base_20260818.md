...
> **2026-08-18 補充更正：**本文件後段引用的 `28%～44%` 是兩段完整 session 的
> `mean(|act_w|)/mean(|sent_w|)`，包含未起步區段、疑似 mux／人工接管區段，且沒有
> 補償 300～400 ms 延遲，因此不可再稱為穩態角速度增益。五個乾淨行駛窗口的對齊
> 增益為 `0.983～1.046`。目前成立的 mismatch 是延遲中心，而不是三倍的尺度落差；
> SA5 保留 scale 1.0，只把 delay `U{0,1,2}` 改為 `U{1,2}`。原文數字保留為歷史
> 稽核軌跡，後續引用以 Gate 筆記 `35_SA5_sim2real_v2從SA4R3_it125啟動_20260818`
> 的原始 CSV 重算為準。
