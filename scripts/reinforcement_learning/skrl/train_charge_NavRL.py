...
"""
Script to train Charge navigation RL agent with SKRL using NavRL-style rewards.

參考 NavRL (https://github.com/Zhefan-Xu/NavRL) 的獎勵函數設計：
- 使用 LiDAR 距離的對數作為安全獎勵
- 速度方向獎勵（鼓勵向目標移動）
- 動作平滑度懲罰（防止抖動）

NavRL 核心思想：通過獎勵函數讓 RL 自己學會避障，而不是使用額外的安全防護罩過濾動作。

環境規格:
    - 觀測空間: 81 維 (Policy) / 131 維 (Critic with privileged info)
...
if headless_mode:
    os.environ["WANDB_PROJECT"] = "charge_skrl_navrl"
    print("[INFO] WandB logging enabled: project = charge_skrl_navrl (NavRL-style rewards)")
else:
...
# Import NavRL custom models
from navrl_models import (
    NavRLPolicy,
    NavRLValue,
    CNNFeatureExtractor,
...
# Register NavRL models with SKRL's model registry
import skrl
...
# Make models available to skrl's model instantiation system
skrl.resources.torch.models.navrl_models = sys.modules[__name__]
...
def patch_skrl_for_aac():
    ...
    def aac_record_transition(self, states, actions, rewards, next_states, terminated, truncated, infos, timestep, timesteps):
        ...
        Key changes:
        1. Extract shared_states from infos (fallback to states)
        2. Compute values using shared_states (131-dim privileged info)
        ...
        if self.memory is not None:
            ...
            # This is needed for computing last_values with privileged info
            ...
            # AAC: Use shared_states (privileged info, 131-dim) for critic
            # Fallback: If shared_states not available, use states (functionally symmetric)
            ...
            with torch.cuda.amp.autocast(enabled=self._mixed_precision):
                ...
                # Skip value_preprocessor for AAC (Critic uses raw privileged info)
            ...
            if hasattr(self, '_current_next_shared_states') and self._current_next_shared_states is not None:
                try:
                    ...
                    if hasattr(self, '_shared_states_preprocessor'):
                        preprocessed_states = self._shared_states_preprocessor(self._current_next_shared_states.float())
                    else:
                        preprocessed_states = self._current_next_shared_states.float()
        ...
        for epoch in range(self._learning_epochs):
            ...
            for batch_data in sampled_batches:
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    # 🔥 AAC CORE FIX: Value Loss uses 131-dim shared_states!
                    # Use separate preprocessor for shared_states if available
                    if has_shared_states:
                        # Check if we have a separate preprocessor for shared_states
                        if hasattr(self, '_shared_states_preprocessor'):
                            sampled_shared_states_for_value = self._shared_states_preprocessor(sampled_shared_states, train=not epoch)
                        else:
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    if headless_mode:
        try:
            import wandb
            wandb.init(
                project="charge_skrl_navrl",
                name=f"Phase0_NavRL_{run_info}",
                config={
                    "task": args_cli.task,
                    ...
                    # NavRL 特定參數
                    "navrl_velocity_weight": args_cli.navrl_velocity_weight,
                    "navrl_safety_weight": args_cli.navrl_safety_weight,
                    "navrl_smoothness_weight": args_cli.navrl_smoothness_weight,
                    "navrl_lidar_range": args_cli.navrl_lidar_range,
                },
            )
            wandb_run = wandb.run
    ...
    if wandb_run is not None:
        ...
        # 创建WandB-enabled trainer（同时传入console_summary_logger）
