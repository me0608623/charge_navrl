"""One-iteration GPU smoke for paper factorial cell A."""
...
CONFIG = replace(
    _FORMAL,
    name="e2e_sa1_paper_factorial_a_ideal_d0_p600_smoke",
    description="64-env one-iteration GPU smoke for paper factorial cell A.",
    num_envs=64,
    timesteps=128,
    save_interval=1,
...
if _actual_diffs != EXPECTED_DIFFS:
    raise RuntimeError(f"factorial A smoke drifted: {sorted(_actual_diffs)}")
