"""One-iteration GPU smoke for paper factorial cell C."""
...
CONFIG = replace(
    _FORMAL,
    name="e2e_sa1_paper_factorial_c_ideal_u012_p600_smoke",
    description="64-env one-iteration GPU smoke for paper factorial cell C.",
    num_envs=64,
    timesteps=128,
    save_interval=1,
...
if _actual_diffs != EXPECTED_DIFFS:
    raise RuntimeError(f"factorial C smoke drifted: {sorted(_actual_diffs)}")
