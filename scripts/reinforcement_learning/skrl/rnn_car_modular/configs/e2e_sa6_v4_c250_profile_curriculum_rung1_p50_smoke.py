"""One-iteration GPU smoke for the SA6 c250 rung-1 intervention."""

from __future__ import annotations

from dataclasses import fields, replace

from rnn_car_modular.configs.e2e_sa6_v4_c250_profile_curriculum_rung1_p50 import (
    CONFIG as _FORMAL,
)
...
CONFIG = replace(
    _FORMAL,
    name="e2e_sa6_v4_c250_profile_curriculum_rung1_p50_smoke",
    description="64-env one-iteration GPU smoke for the SA6 c250 rung-1 arm.",
    num_envs=64,
    timesteps=128,
    save_interval=1,
    tags=_FORMAL.tags + ("gpu_smoke_only",),
    notes=f"{_FORMAL.notes} Runtime smoke only; no capability interpretation.",
)
...
if _actual_diffs != EXPECTED_DIFFS:
    raise RuntimeError(f"SA6 c250 rung-1 smoke drifted: {sorted(_actual_diffs)}")
