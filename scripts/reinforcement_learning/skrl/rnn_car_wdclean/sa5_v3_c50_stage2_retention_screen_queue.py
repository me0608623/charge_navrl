"""Run the frozen 8-cell A2 versus B2 paired-retention screen."""
...
RUNNER = HERE / "run_sa5_v3_c50_stage2_retention_screen_cell.py"
FREEZE = REPO / "docs/freeze/sa5_v3_c50_stage2_retention_screen_v1.json"
SOURCE_PATHS = (
    Path(__file__).resolve(),
    Path(protocol.__file__).resolve(),
    RUNNER,
    FREEZE,
    protocol.PAIRED_SUMMARY,
...
def verify_frozen_protocol() -> dict:
    ...
    if (
        frozen.get("schema")
        != "sa5_v3_c50_stage2_retention_screen_freeze/v1"
        or frozen.get("protocol_sha256") != current["sha256"]
        or frozen.get("paired_summary_sha256")
        != protocol.PAIRED_SUMMARY_SHA256
        or int(frozen.get("cell_count", -1)) != 8
        or bool(frozen.get("starts_training"))
        or bool(frozen.get("starts_sa6"))
    ):
        raise RuntimeError("paired-retention screen differs from frozen protocol")
    return current
...
def build_checkpoint_manifest() -> dict:
    ...
    for spec in protocol.CANDIDATE_SPECS:
        ...
        checkpoints[spec["name"]] = {
            ...
            "motion_weights": spec["motion_weights"],
            "status": spec["status"],
            "rl_optimizer_state_entries": len(optimizer["state"]),
        }
    return {
        "schema": "sa5_v3_c50_stage2_retention_checkpoint_manifest/v1",
...
def render_markdown(summary: dict) -> str:
    lines = [
        "# SA5-v3 stage-2 A2/B2 paired retention screen",
        "",
        f"Status: `{summary['status']}`",
        "",
        "| scenario | A2 SR | A2 CR | B2 SR | B2 CR | dSR | dCR | absolute | relative |",
        "|---|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for row in summary["comparisons"]:
        ...
        lines.append(
            f"| {row['scenario']} | {a2['sr']:.2%} | {a2['cr']:.2%} | "
            f"{b2['sr']:.2%} | {b2['cr']:.2%} | "
            f"{-delta['sr_degradation']:+.2%} | "
            f"{delta['cr_degradation']:+.2%} | "
            f"{'PASS' if b2['absolute_pass'] else 'FAIL'} | "
            f"{'PASS' if row['relative']['pass'] else 'FAIL'} |"
        )
    lines.extend(
        [
            "",
            f"Retention supported: `{summary['retention_supported']}`",
            f"Next action: `{summary['next_action']}`",
            "",
            "No extension, graduation, parent promotion, or SA6 start is authorized.",
        ]
    )
    return "\n".join(lines) + "\n"
