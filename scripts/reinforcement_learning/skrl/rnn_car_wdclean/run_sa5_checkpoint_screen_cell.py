"""Run one frozen SA5 checkpoint-screen cell."""
...
def build_scene_args(scenario: str, corridor_json: Path) -> list[str]:
    values = stage_values()
    ...
    if scenario in protocol.CORRIDOR_SCENARIOS:
        speed = values["corridor_speed_range_m_s"]
        return [
            *common,
            "--long_corridor_eval",
            ...
            str(values["corridor_static_obstacles"]),
            "--long_corridor_dynamic_obstacles",
            str(values["corridor_dynamic_obstacles"]),
            "--long_corridor_motion_mode",
            MOTION_MODE[scenario],
...
def verify_corridor_runtime(report: dict, scenario: str, values: dict) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    values = stage_values()
    print(
        f"[SA5-SCREEN-CELL] checkpoint={candidate['name']} "
    ...
    cell = {
        "schema": "sa5_checkpoint_screen_cell/v1",
        "cell_valid": True,
    ...
    print(
        f"[SA5-SCREEN-CELL] {candidate['name']} {args.scenario} "
