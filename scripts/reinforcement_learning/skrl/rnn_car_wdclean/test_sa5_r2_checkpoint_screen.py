...
import sa5_r2_checkpoint_screen as screen  # noqa: E402
import sa5_r2_checkpoint_screen_queue as queue  # noqa: E402
import run_sa5_r2_checkpoint_screen_cell as runner  # noqa: E402


FREEZE = REPO / "docs/freeze/sa5_r2_checkpoint_screen_v1.json"
RUNNER = HERE / "run_sa5_r2_checkpoint_screen_cell.py"
QUEUE = HERE / "sa5_r2_checkpoint_screen_queue.py"
...
def test_candidate_identity_and_two_phase_matrix_are_frozen():
    ...
    assert protocol["phase_a"]["cells"] == 24
    assert protocol["phase_b"]["cells"] == 8
    ...
    assert fixed["phase_a_corridor_fixed_density"] == "4S2D"
    assert fixed["phase_b_low_density"] == {
...
def test_ranking_uses_the_worst_family_before_the_mean():
    payloads = _phase_a(
        {
            "c50": (0.01, 0.01, 0.01, 0.09),
            "c100": (0.06, 0.06, 0.06, 0.06),
            "c150": (0.07, 0.01, 0.01, 0.01),
            "c200": (0.08, 0.01, 0.01, 0.01),
            "c250": (0.09, 0.01, 0.01, 0.01),
            "c300": (0.10, 0.01, 0.01, 0.01),
        }
    )
    result = screen.rank_phase_a(payloads)
    assert result["ranked"] == [
        "c100", "c150", "c200", "c250", "c50", "c300"
    ]
    assert result["top_two"] == ["c100", "c150"]


def test_hard_gate_failure_is_reported_but_does_not_erase_ranking_data():
    payloads = _phase_a(
        {
            "c50": (0.11, 0.01, 0.01, 0.01),
            "c100": (0.12, 0.12, 0.12, 0.12),
            "c150": (0.13, 0.13, 0.13, 0.13),
            "c200": (0.14, 0.14, 0.14, 0.14),
            "c250": (0.15, 0.15, 0.15, 0.15),
            "c300": (0.16, 0.16, 0.16, 0.16),
        }
    )
    payloads[0]["threshold_pass"] = False
    result = screen.rank_phase_a(payloads)
    assert result["ranked"][0] == "c50"
    row = next(row for row in result["summaries"] if row["checkpoint_name"] == "c50")
...
def test_missing_or_invalid_phase_a_cell_fails_closed():
    payloads = _phase_a(
        {
            name: (0.05, 0.05, 0.05, 0.05)
            for name in ("c50", "c100", "c150", "c200", "c250", "c300")
        }
    )
    try:
...
def test_phase_b_only_folds_the_selected_top_two_and_never_accepts_parent():
    phase_a = _phase_a(
        {
            "c50": (0.04, 0.04, 0.04, 0.04),
            "c100": (0.05, 0.05, 0.05, 0.05),
            "c150": (0.06, 0.06, 0.06, 0.06),
            "c200": (0.07, 0.07, 0.07, 0.07),
            "c250": (0.08, 0.08, 0.08, 0.08),
            "c300": (0.09, 0.09, 0.09, 0.09),
        }
    )
    phase_b = [
    ...
    assert verdict["phase_b_candidates"][0]["all_eight_hard_pass"] is True
...
def test_low_density_cells_are_sealed_mixed_0s1d_and_1s1d(tmp_path):
    values = runner.stage_values()
    for scenario, expected in (
        ("corridor_low_0s1d", (0, 1)),
        ("corridor_low_1s1d", (1, 1)),
    ):
        args = runner.build_scene_args(scenario, tmp_path / f"{scenario}.json")
        static_index = args.index("--long_corridor_static_obstacles") + 1
        dynamic_index = args.index("--long_corridor_dynamic_obstacles") + 1
        motion_index = args.index("--long_corridor_motion_mode") + 1
        assert (int(args[static_index]), int(args[dynamic_index])) == expected
        assert args[motion_index] == "mixed"

        report = _sealed_corridor_report()
        report["static_obstacles_per_env"] = expected[0]
        report["dynamic_obstacles_per_env"] = expected[1]
        report["dynamic_motion_mode"] = "mixed"
        report["dynamic_motion_type_fractions"] = {
            "lateral": 0.34,
            "longitudinal": 0.33,
            "random_2d": 0.33,
        }
        result = runner.verify_corridor_runtime(report, scenario, values)
        assert result["fixed_density"] == f"{expected[0]}S{expected[1]}D"


def test_low_density_smoothness_is_descriptive_not_a_post_hoc_gate():
    fixed = screen.screen_protocol()["fixed_evaluation"]
    diagnostics = fixed["low_density_behavior_diagnostics"]
    assert set(diagnostics) == {
        "linear_speed_abs_mean_mps",
        "stop_command_fraction",
    }
    assert all("descriptive" in value for value in diagnostics.values())
    assert screen.thresholds_for("corridor_low_0s1d") == screen.CORRIDOR_THRESHOLDS
