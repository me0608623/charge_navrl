"""Run the frozen 8-cell direction decomposition: c50 anchor vs pilot it50 at 4S2D."""
...
RUNNER = HERE / "run_sa5_v3_c50_it50_direction_ab_cell.py"
FREEZE = REPO / "docs/freeze/sa5_v3_c50_it50_direction_ab_v1.json"
SOURCE_PATHS = (
...
def build_checkpoint_manifest() -> dict:
    ...
    return {
        "schema": "sa5_v3_c50_it50_direction_ab_checkpoint_manifest/v1",
...
def render_markdown(summary: dict) -> str:
    lines = [
        "# SA5-v3 c50 vs pilot it50 — 4S2D direction decomposition",
        "",
        f"Status: `{summary['status']}`",
        "",
        "Primary metrics are **TO and SR**; CR is secondary.",
        "",
        "| motion mode | arm | n | SR | CR | TO | stop | speed | reverse |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in summary["rows"]:
        ...
        lines.append(
            ...
            f"{fmt(b['stop_command_fraction'])} | "
            f"{fmt(b['linear_speed_abs_mean_mps'])} | "
            f"{fmt(b['reverse_command_fraction'])} |"
        )
    lines.extend(
        [
            "",
            "## it50 minus anchor (primary metrics)",
            "",
            "| motion mode | dTO | z(TO) | dSR | z(SR) | dCR | z(CR) | verdict |",
            "|---|---:|---:|---:|---:|---:|---:|---|",
        ]
    )
    for row in summary["directions"]:
        to_c, sr_c, cr_c = row["primary"]["to"], row["primary"]["sr"], row["secondary"]["cr"]
        verdict = (
            "DEGRADED" if row["degraded"] else ("IMPROVED" if row["improved"] else "flat")
        )
        lines.append(
            f"| {row['motion_mode']} | {to_c['delta']:+.2%} | {to_c['z']:+.1f} | "
            f"{sr_c['delta']:+.2%} | {sr_c['z']:+.1f} | "
            f"{cr_c['delta']:+.2%} | {cr_c['z']:+.1f} | {verdict} |"
        )
    lines.extend(
        [
            "",
            "## Anchor replication vs difficulty frontier",
            "",
            f"Exact: `{summary['anchor_replication_exact']}`",
            "",
            "| motion mode | reference CR | observed CR | reference n | observed n | identical |",
            "|---|---:|---:|---:|---:|---|",
        ]
    )
    for row in summary["anchor_replication"]:
        lines.append(
            f"| {row['motion_mode']} | {row['reference_cr']:.2%} | "
            f"{row['observed_cr']:.2%} | {row['reference_n']} | "
            f"{row['observed_n']} | {row['identical']} |"
        )
    lines.extend(
        [
            "",
            f"Degraded modes: `{summary['degraded_motion_modes']}`",
            f"Improved modes: `{summary['improved_motion_modes']}`",
            f"Next action: `{summary['next_action']}`",
            "",
            "This evaluation-only screen started no training and authorizes no graduation.",
        ]
    )
    return "\n".join(lines) + "\n"
