"""Run the frozen 18-cell SA4-v3 checkpoint screen."""
...
RUNNER = HERE / "run_sa4_v3_checkpoint_screen_cell.py"
...
def source_fingerprint() -> dict:
    ...
    return {
        "schema": "sa4_v3_checkpoint_screen_fingerprint/v1",
        "files": files,
...
def render_markdown(summary: dict) -> str:
    headers = [spec["label"] for spec in protocol.FIXED_CELLS]
    lines = [
        "# SA4-v3 checkpoint screen",
        "",
        f"Status: `{summary['status']}`",
        f"Protocol: `{summary['protocol_sha256']}`",
        "",
        "| rank | checkpoint | "
        + " | ".join(f"{label} SR/CR/TO" for label in headers)
        + " | worst CR | mean CR | hard gate |",
        "|---:|---|"
        + "---|" * len(headers)
        + "---:|---:|---|",
    ]
    for rank, row in enumerate(summary["ranked_candidates"], start=1):
        ...
        for label in headers:
            ...
            rendered_cells.append(
                f"{cell['sr']:.2%}/{cell['cr']:.2%}/{cell['to']:.2%}"
            )
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            "No retention or continuation was started; no SA5 parent was accepted; SA5 was not started.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    completed: list[dict] = []
    try:
        for index, spec in enumerate(protocol.cells(), start=1):
            ...
            print(
                f"[SA4-V3-SCREEN-QUEUE] START {index}/{len(protocol.cells())} "
                f"{label}",
                flush=True,
            )
            env = os.environ.copy()
            ...
            print(
                f"[SA4-V3-SCREEN-QUEUE] DONE {label} "
        ...
        print(
            "[SA4-V3-SCREEN-QUEUE] COMPLETE promoted="
            f"{summary['promoted_for_retention']} retention_started=false "
            "continuation_started=false sa5_started=false",
            flush=True,
        )
        return 0
    except Exception as exc:
        incomplete = {
            "schema": "sa4_v3_checkpoint_screen_incomplete/v1",
