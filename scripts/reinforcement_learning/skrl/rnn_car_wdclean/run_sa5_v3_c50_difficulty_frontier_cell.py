"""Run one frozen SA5-v3 c50 difficulty-frontier cell."""
...
_MOTION_MODE = {
    row["name"]: row["motion_mode"] for row in protocol.SCENARIO_SPECS
}
_DENSITY = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"difficulty-frontier runner produced {len(matches)} cell JSON files"
        )
    path = matches[0]
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["schema"] = protocol.CELL_SCHEMA
    payload["anchor_status"] = protocol.ANCHOR["status"]
