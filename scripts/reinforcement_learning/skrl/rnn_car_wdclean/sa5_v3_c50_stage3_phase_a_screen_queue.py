"""Run the frozen 8-cell A3/B3 P060/P080 Phase-A screen."""
...
import sa5_v3_c50_stage3_phase_a_screen as protocol  # noqa: E402
import sa5_v3_provisional_pilot_screen_queue as base_queue  # noqa: E402


RUNNER = HERE / "run_sa5_v3_c50_stage3_phase_a_screen_cell.py"
FREEZE = REPO / "docs/freeze/sa5_v3_c50_stage3_phase_a_screen_v1.json"
SOURCE_PATHS = (
    ...
    "e2e_sa5_v3_c50_stage3_a3_control_p25.py",
    REPO
    / "scripts/reinforcement_learning/skrl/rnn_car_modular/configs/"
...
def build_checkpoint_manifest() -> dict:
    ...
    return {
        "schema": "sa5_v3_c50_stage3_phase_a_checkpoint_manifest/v1",
...
def render_markdown(summary: dict) -> str:
    lines = [
        "# SA5-v3 Stage-3 A3/B3 Phase-A screen",
        "",
        f"Status: `{summary['status']}`",
        "",
        "| scenario | A3 SR | A3 CR | B3 SR | B3 CR | dSR | dCR | B3 absolute | relative |",
        "|---|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for row in summary["comparisons"]:
        a3 = row["a3_control"]
        b3 = row["b3_p060_aligned"]
        delta = row["relative"]["deltas"]
        lines.append(
            f"| {row['scenario']} | {a3['sr']:.2%} | {a3['cr']:.2%} | "
            f"{b3['sr']:.2%} | {b3['cr']:.2%} | "
            f"{-delta['sr_degradation']:+.2%} | {delta['cr_degradation']:+.2%} | "
            f"{'PASS' if b3['absolute_pass'] else 'FAIL'} | "
            f"{'PASS' if row['relative']['pass'] else 'FAIL'} |"
        )
    worst = summary["worst_p060_cr"]
    lines.extend(
        [
            "",
            f"Worst P060 CR improvement: `{worst['improvement']:+.2%}` "
            f"(required `{worst['minimum_required']:.2%}`)",
            f"P060 absolute pass: `{summary['p060_absolute_pass']}`",
            f"P080 retention pass: `{summary['p080_retention_pass']}`",
            f"Phase-A pass: `{summary['phase_a_pass']}`",
            f"Next action: `{summary['next_action']}`",
            "",
            "No extension, graduation, parent selection, or SA6 start is authorized.",
        ]
    )
    return "\n".join(lines) + "\n"
