"""Run the frozen SA5-R2 corridor ranking, then top-two retention screen.
...
def wait_for_resources(
    ...
    *,
    ...
    while True:
        ...
        print(
            f"[SA5-R2-SCREEN-WAIT] mode={snapshot['policy_mode']} "
...
def _render_markdown(verdict: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "## Phase B native, narrow, and low-density retention",
            "",
            "| checkpoint | native SR | native CR | narrow SR | narrow CR | crossing | direct | all eight gates |",
            "|---|---:|---:|---:|---:|---:|---:|---|",
        ]
    )
    for row in verdict["phase_b_candidates"]:
        ...
        lines.append(
            ...
            f"{'PASS' if row['all_eight_hard_pass'] else 'FAIL'} |"
        )
    lines.extend(
        [
            "",
            "## Phase B sealed low-density corridor",
            "",
            "| checkpoint | density | SR | CR | TO | mean speed | stop fraction | outcome gate |",
            "|---|---|---:|---:|---:|---:|---:|---|",
        ]
    )
    for row in verdict["phase_b_candidates"]:
        for scenario in protocol.LOW_DENSITY_SCENARIOS:
            metrics = row["low_density"][scenario]
            lines.append(
                f"| {row['checkpoint_name']} | "
                f"{scenario.removeprefix('corridor_low_').upper()} | "
                f"{metrics['sr']:.4%} | {metrics['cr']:.4%} | "
                f"{metrics['to']:.4%} | "
                f"{metrics['linear_speed_abs_mean_mps']:.4f} m/s | "
                f"{metrics['stop_command_fraction']:.4%} | "
                f"{'PASS' if metrics['sr'] >= 0.90 and metrics['cr'] <= 0.10 and metrics['to'] <= 0.05 else 'FAIL'} |"
            )
    lines.extend(
        [
            "",
            ...
            "",
            "Speed and stop fraction are preregistered descriptive diagnostics, not post-hoc gates.",
            "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"[SA5-R2-SCREEN] Phase A {len(cells)} cells = "
        f"{len(protocol.CANDIDATES)} checkpoints x "
        f"{len(protocol.CORRIDOR_SCENARIOS)} families",
        flush=True,
    )
    print(
        f"[SA5-R2-SCREEN] fixed seed={protocol.SEED} d{protocol.DELAY_STEPS} "
    ...
    if not args.execute:
        print("[SA5-R2-SCREEN] dry run; pass --execute to run")
        return 0
    ...
    try:
        ...
        for index, cell in enumerate(cells, start=1):
            ...
            print(
                f"[SA5-R2-SCREEN] Phase A cell {index}/{len(cells)}: "
                f"{cell['checkpoint_name']} {cell['scenario']}",
                flush=True,
            )
        ...
        phase_a_summary = {
            "schema": "sa5_r2_checkpoint_screen_phase_a/v1",
        ...
        print(
            f"[SA5-R2-SCREEN] Phase A ranked={phase_a_result['ranked']} "
        ...
        for index, cell in enumerate(phase_b_cells, start=1):
            ...
            print(
                f"[SA5-R2-SCREEN] Phase B cell {index}/{len(phase_b_cells)}: "
                f"{cell['checkpoint_name']} {cell['scenario']}",
                flush=True,
            )
        ...
        summary = {
            "schema": "sa5_r2_checkpoint_screen_bundle/v1",
        ...
        _write_json(
            paths["incomplete"],
            {
                "schema": "sa5_r2_checkpoint_screen_incomplete/v1",
