"""Run one frozen SA5-v3 Stage-3 Phase-A P060/P080 cell."""
...
implementation.implementation.MOTION_MODE = dict(implementation.implementation.MOTION_MODE)
implementation.implementation.DENSITY = dict(implementation.implementation.DENSITY)
for _scenario in protocol.ALL_SCENARIOS:
    implementation.implementation.MOTION_MODE[_scenario] = "lateral"
    implementation.implementation.DENSITY[_scenario] = protocol.density_for(_scenario)
...
def verify_corridor_runtime(
    ...
    **_: object,
    ...
    expected["corridor_speed_range_m_s"] = list(protocol.speed_range_for(scenario))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"Stage-3 Phase-A runner produced {len(matches)} cell JSON files"
        )
    path = matches[0]
