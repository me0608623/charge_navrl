"""Run the frozen 8-cell A2 equal versus B2 mild paired screen."""
...
RUNNER = HERE / "run_sa5_v3_c50_stage2_paired_screen_cell.py"
...
SOURCE_PATHS = (
    ...
    protocol.A2_COMPLETION,
    protocol.B2_COMPLETION,
...
def verify_frozen_protocol() -> dict:
    ...
    if (
        frozen.get("schema") != "sa5_v3_c50_stage2_paired_screen_freeze/v1"
        or frozen.get("protocol_sha256") != current["sha256"]
        or frozen.get("authorization_sha256") != protocol.AUTHORIZATION_SHA256
        or frozen.get("a2_completion_sha256") != protocol.A2_COMPLETION_SHA256
        or frozen.get("b2_completion_sha256") != protocol.B2_COMPLETION_SHA256
    ):
        raise RuntimeError("paired screen differs from frozen protocol")
    return current
...
def build_checkpoint_manifest() -> dict:
    ...
    return {
        "schema": "sa5_v3_c50_stage2_paired_screen_checkpoint_manifest/v1",
...
def render_markdown(summary: dict) -> str:
    lines = [
        "# SA5-v3 stage-2 equal vs 30/30/40 paired screen",
        "",
        f"Status: `{summary['status']}`",
        "",
        "| motion | A2 SR | A2 CR | B2 SR | B2 CR | dSR | zSR | dCR | zCR |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in summary["comparisons"]:
        lines.append(
            f"| {row['motion_mode']} | {row['a2_equal']['sr']:.2%} | "
            f"{row['a2_equal']['cr']:.2%} | {row['b2_mild']['sr']:.2%} | "
            f"{row['b2_mild']['cr']:.2%} | {row['sr']['delta']:+.2%} | "
    ...
    lines.extend(
        [
            ...
            f"Mild weighting accepted: `{summary['mild_weight_acceptance_observed']}`",
            f"Next action: `{summary['next_action']}`",
            "",
            "Single training seed paired diagnostic. No extension, graduation, or SA6 is authorized.",
        ]
    )
    return "\n".join(lines) + "\n"
