"""Run and compare the six frozen SA4-R3 continuation screen cells."""
...
import sa4_r3_cont50_checkpoint_screen as protocol  # noqa: E402


RUN_NAME = "sa4_r3_cont50_from_r3c6400_ne1024_s42_p50_r1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"[SA4-R3-CONT50-SCREEN] {len(planned)} cells, seed={protocol.SEED}, "
        f"d{protocol.DELAY_STEPS}, eligibility={protocol.ELIGIBILITY}"
    )
    for cell in planned:
        print(
            f"  {cell['checkpoint_name']:13s} {cell['scenario']:22s} "
    ...
    if busy:
        raise RuntimeError(f"SA4-R3 continuation screen refuses busy GPU: {busy}")

    output = args.output_root.expanduser().resolve()
    output.mkdir(parents=True, exist_ok=True)
    common_paths = {
        "protocol": output / "PROTOCOL.json",
        "summary": output / "sa4_r3_cont50_checkpoint_screen_summary.json",
        "incomplete": output / "SA4_R3_CONT50_SCREEN_INCOMPLETE.json",
    ...
    try:
        for index, cell in enumerate(planned, start=1):
            print(
                f"[SA4-R3-CONT50-SCREEN] cell {index}/{len(planned)} "
        ...
        summary = {
            "schema": "sa4_r3_cont50_checkpoint_screen_bundle/v1",
        ...
        common_paths["incomplete"].write_text(
            json.dumps(
                {
                    "schema": "sa4_r3_cont50_checkpoint_screen_incomplete/v1",
