"""Frozen protocol and selection rule for the SA4-R3 continuation screen."""
...
PROTOCOL_SCHEMA = "sa4_r3_cont50_checkpoint_screen/v1"
CHECKPOINTS = (
    ("sa4_r3_it75", "checkpoint_3200.pt", 75),
    ("sa4_r3_it100", "checkpoint_6400.pt", 100),
)
...
def screen_protocol() -> dict:
    protocol = {
        ...
        "purpose": (
            "screen conceptual SA4-R3 it75 and it100 after an exact optimizer "
            "continuation on lateral, longitudinal, and native stage-4 capabilities"
        ),
        "checkpoints": [
        ...
        "authorization_limit": (
            "the screen selects evidence only; it starts no additional training or SA5"
        ),
    }
...
def compare_checkpoints(payloads: list[dict]) -> dict:
    ...
    return {
        "schema": "sa4_r3_cont50_checkpoint_comparison/v1",
        "rows": rows,
        "qualifying_candidates": [row["checkpoint_name"] for row in candidates],
        "selected_checkpoint": selected,
        "sa5_consideration_eligible": selected is not None,
