"""Run one frozen c50 difficulty-frontier pilot acceptance cell."""
...
def _bind_protocol() -> None:
    """Rebind shared implementation after pytest imports sibling runners."""
    implementation.protocol = protocol
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"frontier-pilot runner produced {len(matches)} cell JSON files"
        )
    path = matches[0]
